    ...
    public class SlackHelper
    {
        ...
        private void OnMessageReceived(SlackAPI.WebSocketMessages.NewMessage message)
        {
            if (!string.IsNullOrEmpty(message.user) && message.user != client.MySelf.id)
            {
                ...
                if (message.user == adminId && message.channel == adminDM)
                {
                    ...
                    else if (messageText == "help")
                    {
                        var helpMessage = "Hey Connor, here's what you can tell me:\n" +
                                          ...
                                          "Delete all attempts by saying 'delete history 8675309'.\n" +
                ...
                {
                    if (message.channel != "C6ZPBU15W" && (messageText != "a" && messageText != "b" && messageText != "c" && messageText != "d"))
                    {
                        var speakingPlayer = LookUpPlayer(message.user);
                        PostMessage(adminId, "*Message received from " + speakingPlayer.PlayerMention + ":* " + messageText);
                    }
        ...
        string RecordAnswer(NewMessage message, string messageText)
        {
            ...
            {
                PostMessage(adminDM, currentPlayer.PlayerFirstName + " " + currentPlayer.PlayerLastName + " tried to answer again but I found an answer for them already.");
                return ("Thanks " + currentPlayer.PlayerFirstName + "! You answered " + messageText.ToUpper() + ". Your answer has been...wait a second, didn't you already answer this one?");
            }
        ...
        Player CreatePlayer(string playerId, string directMessageChannel)
        {
            ...
            newPlayer = new Player
            {
                ...
                PlayerFirstName = user.profile.first_name,
                PlayerLastName = user.profile.last_name,
        ...
        string CheckAllScores()
        {
            ...
            else
            {
                foreach (var player in playerList)
                {
                    ...
                    scoreSheet += player.PlayerFirstName + " " + player.PlayerLastName + ": " + score + "\n";
                }
                return scoreSheet;
            }
        }
        ...
        void DeleteMostRecentPlayerAttempt(string command)
        {
            ...
            dbcontext.SaveChanges();
            PostMessage(adminDM, "Deleted " + player.PlayerFirstName + " " + player.PlayerLastName + "'s last attempt.");
        }
        ...
        string CheckForAndAddDM(string playerId, string playerDM)
        {
            ...
            else if (string.IsNullOrWhiteSpace(player.DirectMessage))
            {
                ...
                dbcontext.SaveChanges();
                return $"Added DM to {player.PlayerMention}'s profile.";
            }
            else
            {
                return "DM exists";
        ...
        //Tell @username Here's the sentence I want to send.
        {
            ...
            PostMessage(adminDM, $"*Telling {player.PlayerFirstName}:* {message2}");
        ...
        //add attempt for @username a
        {
            try
            {
                ...
                if (dbcontext.Attempts.FirstOrDefault(x => x.Question.QuestionId == question.QuestionId && x.Player.PlayerId == currentPlayer.PlayerId) != null)
                {
                    return ($"Attempt already created for {currentPlayer.PlayerFirstName}");
                }
                ...
                return $"{currentPlayer.PlayerFirstName}'s guess added. Previous score was {previousScore}. Current score is {postScore}";
            }
            catch (Exception ex)
            {
